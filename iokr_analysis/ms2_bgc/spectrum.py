...
def ppk_wrapper(ms_i, ms_j):
    sigma_mass = 0.00001
    sigma_int = 1000000.0
    ppk_peaks_value = ppk(ms_i.spectrum, ms_j.spectrum, sigma_mass, sigma_int)
    ppk_nloss_value = ppk_nloss(ms_i.spectrum, ms_j.spectrum, ms_i.parentmass, ms_j.parentmass, sigma_mass, sigma_int)

    if not hasattr(ms_i, 'ppk_peaks'):
        ms_i.ppk_peaks = ppk(ms_i.spectrum, ms_i.spectrum, sigma_mass, sigma_int)
        ms_i.ppk_nloss = ppk_nloss(ms_i.spectrum, ms_i.spectrum, ms_i.parentmass, ms_i.parentmass, sigma_mass, sigma_int)

    if not hasattr(ms_j, 'ppk_peaks'):
        ms_j.ppk_peaks = ppk(ms_j.spectrum, ms_j.spectrum, sigma_mass, sigma_int)
        ms_j.ppk_nloss = ppk_nloss(ms_j.spectrum, ms_j.spectrum, ms_j.parentmass, ms_j.parentmass, sigma_mass, sigma_int)

    ppk_peaks_normalised = ppk_peaks_value / numpy.sqrt(ms_i.ppk_peaks * ms_j.ppk_peaks)
    ppk_nloss_normalised = ppk_nloss_value / numpy.sqrt(ms_i.ppk_nloss * ms_j.ppk_nloss)
    ppk_value = (ppk_peaks_normalised + ppk_nloss_normalised) / 2
    return ppk_value
...
class MS2(object):
    def __init__(self, spectrum_id, spectrum, metabolite):
...
def load_data_dir(spectra_file, spectra_path, filter_function):
    ms2_list = []
    with open(spectra_file, 'r') as f:
        header = next(f)
        for line in f.readlines():
            spectrum_id, name, inchi = line.strip().split('\t')
            ms2 = MSSpectrum()
            ms2.correct_for_ionisation = True
            ms2.normalise = True
            # ms2.filter = spectrum_filters.filter_by_frozen_dag
            ms2.filter = filter_function
            ms2.load(os.path.join(spectra_path, spectrum_id + '.ms'))

            ms2_obj = MS2(spectrum_id, ms2, inchi)
            ms2_list.append(ms2_obj)
    return ms2_list
